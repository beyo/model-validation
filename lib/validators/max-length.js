...
*/
module.exports = function * maxLength(propertyName, options, translator) {
  var value = this._data[propertyName];
  var maxLen = optUtil.getInteger(options, 'max', DEFAULT_MAX_LENGTH);

  if ((typeof value !== 'string') || (value.length > maxLen)) {
    return (options && options.message || format(yield translator(DEFAULT_MESSAGE), propertyName, maxLen));
  }
};
