...
*/
module.exports = function * unique(propertyName, options, translator) {
  ...
  var searchCallback = optUtil.getFunction(options, 'searchCallback', defaultSearchCallback(this));
  var filter;
  var otherModel;

  if (isUnique && searchCallback) {
    filter = filterBuilder(propertyName, this);
