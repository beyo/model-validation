...
*/
module.exports = function decimal(model, propertyName, options, translator, next) {
  var positive  = optUtil.getBoolean(options, 'positive', DEFAULT_POSITIVE),
      ...
      value     = getDecimal(model._data[propertyName]);
  ...
  if (!(positive || negative || zero) ||
    ...
    return (options && options.message || format(yield translator(DEFAULT_MESSAGE), propName));
  }
