...
Model.on('define', function modelDefine(options) {
  ...
  options.prototype.validate = {
    ...
    value: function * validate() {
      return yield validateModel(this);
    }
  };

  keys = Object.keys(options.options.attributes || {});
  for (i = 0, ilen = keys.length; i < ilen; ++i) {
    attr = keys[i];
    if (options.attributes[attr] && options.options.attributes[attr].validation) {
      options.attributes[attr].validation = createValidationChain(options.options.attributes[attr].validation, translator);
    }
  }
});
...
function * validateModel(model) {
  ...
  attributes = model.__proto__.constructor.attributes || {};
  ...
  for (i = 0, ilen = keys.length; i < ilen; ++i) {
    ...
    //console.log("Attr", attr, attributes[attr].validation);
    ...
    if (attributes[attr].validation) {
      ...
      //console.log("Chain result for", attr, "is", chainResult);
...
function createValidationChain(validationChain, translator) {
  ...
  return function * validate(model, propertyName) {
    ...
    while (i--) {
      key = keys[i];
      validator = validatorNameSanitize(key);
      ...
      prev = curr.call(this, model, propertyName, validationChain[key], translator, prev);
    }
